...
if (! $copyalreadydone) {
	...
	print "Recopie de $SOURCE/bin dans $BUILDROOT/$PROJECT/bin\n";
	mkdir "$BUILDROOT/$PROJECT/bin";
	$ret=`cp -pr "$SOURCE/bin" "$BUILDROOT/$PROJECT"`;
}


print "Nettoyage de $BUILDROOT\n";
$ret=`rm -f $BUILDROOT/$PROJECT/ChangeLog`;
$ret=`rm -f $BUILDROOT/$PROJECT/bin/$PROJECT.conf`;
$ret=`rm -f $BUILDROOT/$PROJECT/bin/$PROJECT.chiensderace.conf`;
$ret=`rm -f $BUILDROOT/$PROJECT/bin/$PROJECT.sho*.conf`;
$ret=`rm -f $BUILDROOT/$PROJECT/bin/$PROJECT.mytest.conf`;
$ret=`rm -f $BUILDROOT/$PROJECT/bin/$PROJECT.*com.conf`;
$ret=`rm -f $BUILDROOT/$PROJECT/bin/$PROJECT.*net.conf`;
$ret=`rm -f $BUILDROOT/$PROJECT/bin/$PROJECT??????.txt`;
$ret=`rm -f $BUILDROOT/$PROJECT/bin/$PROJECT*.athena.*`;
$ret=`rm -f $BUILDROOT/$PROJECT/bin/buildconffromsnif.pl`;
$ret=`rm -f $BUILDROOT/$PROJECT/bin/proxy.pl`;
$ret=`rm -f $BUILDROOT/$PROJECT/bin/*addon*`;
$ret=`rm -f $BUILDROOT/$PROJECT/bin/output/*`;
$ret=`rm -fr $BUILDROOT/$PROJECT/make`;
$ret=`rm -fr $BUILDROOT/$PROJECT/perl`;
